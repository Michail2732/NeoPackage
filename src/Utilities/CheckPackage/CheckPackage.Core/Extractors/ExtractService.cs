...
namespace CheckPackage.Core.Extractors
{
    public class ExtractService : IExtractService
    {
        ...
        public IEnumerable<Parameter> Extract(IEnumerable<Parameter> source, IEnumerable<ParameterExtractCommand> extracters)
        {
        ...
        {
            var context = _contextBuilder.Build();            
            var parameters = extracter.ExtractParameters(new[] { source}, context);
            if (parameters.Count() == 0)
                _logger.LogError(context.Messages[MessageKeys.CouldNotExtractParameters]);
            return parameters;
        }

        public IEnumerable<Parameter> Extract(Parameter source, IEnumerable<ParameterExtractCommand> extracters)
        {
            var context = _contextBuilder.Build();
            List<Parameter> result = new List<Parameter>();
            IEnumerable<Parameter> parameters = new[] { source };
            foreach (var extracter in extracters)
            {
                parameters = Extract(parameters, extracter);
                if (parameters.Count() > 0)
